...
# Isolating the columns that are needed for the analysis
cleaned_data <- cleaned_data |>
  select(arrest_year, arrest_month, perceived_race, age_group_at_arrest, youth_at_arrest_under_18_years, booked)

# Renaming the columns
cleaned_data <- cleaned_data |>
  rename(youth = youth_at_arrest_under_18_years, age_group = age_group_at_arrest)
...
ggplot(
  table1, aes(x = `perceived_race`, y = `2020`, fill = `2021`)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Number of arrests by ethinic background for the year 2020, and 2021",
       x = "Ethinic and Identity Based Groups",
       y = "Number of Arrests") +
  theme_minimal() +
