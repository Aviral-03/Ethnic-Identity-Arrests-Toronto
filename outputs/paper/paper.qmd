...
As seen in Figure 3, the data indicates a non-uniform distribution of arrests across age groups, with the age group of 25 to 34 years and age group 35 to 44 exhibiting the highest number of arrests in both 2020 and 2021. Furthermore, the data indicates a decrease in the number of arrests for the top two age groups from 2020 to 2021. However, a contrasting trend emerges for other age groups, where the number of arrests has increased over the same time period.
...
table3_age_24_long <- pivot_longer(table3_small, cols = c("2020", "2021"), names_to = "year", values_to = "value")

age_35_44 <- cleaned_data %>%
  filter(age_group == "Aged 35 to 44 years") |>
  group_by(arrest_year, perceived_race, age_group) %>%
  summarise(n = n(), .groups = "drop") |>
  arrange(desc(n)) |>
  pivot_wider(names_from = arrest_year, values_from = n, values_fill = 0) |>
...
# Combining the data
table3_age_24_long <- rbind(table3_age_24_long, age_35_44)

ggplot(table3_age_24_long, aes(x = perceived_race, y = value, fill = factor(year))) +
  geom_bar(stat = "identity", position = 'dodge') +
